...
$TotPoint=filter_input(INPUT_POST,'TotPoint',FILTER_VALIDATE_INT);
...
if($type==1) $Value = $pb['value'];
else $Value =$prodbonus;
if($ProdSeaC && $qte):
...
if($TotEch<=$Quota && $Quota >0 && ($TotEch+$qte*$Value)<=$Quota){
    ...
    if($type==1): // produits
        ...
        if(isset($_SESSION['ProdPbCmd'][$ProdSeaC]) && $_SESSION['ProdPbCmd'][$ProdSeaC]>0){
            ...
            $_SESSION['TotalCdx']+=$qte*$pb['value'];
    ...
    echo "<h3>Total point bonus : ".$TotEch." / $Quota </h3>";
    ...
    <table class="table">
        ...
        foreach ($_SESSION['ProdPbCmd'] as $key=>$value):?>
            <tr>
                ...
                <td><? echo $value?></td>
                <td>
                    <a href="DmdPb&idDemande=<?=$client?>&IdSup=<?=$key?>&prod=1" class="btn btn-danger">
        ...
        foreach ($_SESSION['CdxCmd'] as $key=>$value):?>
            <tr>
                ...
                <td><? echo $value?></td>
                <td>
                    <a href="DmdPb&idDemande=<?=$client?>&IdSup=<?=$key?>" class="btn btn-danger">
                        <i class="fa fa-trash"></i>
                    </a>
                </td>
            </tr>
