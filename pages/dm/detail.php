...
<section class="content-header">
    <h1 class="pull-left">Demande Détail
    ...
    <a href="Liste-demandes" class="btn btn-github pull-right" style="margin-right: 10px ">Retour</a>
...
<section class="content" id="DivToPrint">
   ...
   <h3 class="profile-username text-center"><?=$profile['Civilite'].' '.$profile['Nom']." ".$profile['Prenom']?> </h3>
                    ...
                    <span class="center-block text-center"><b>Ref : <?=getinfo(getinfo($DetailInfo['type'], 'type_demande', 'departement'), 'departement', 'abr').'/'. $DetailInfo['id'].'/'.date("Y", strtotime($DetailInfo['date'])) ?></b></span>
    ...
    <div class="col-md-5">
        ...
        <div class="box-body">
            <ul>
               ...
               <?
               // récupération des prod associer :
               $Prod=get("*",'demande_prod',array('demande_id='=>$id));
               if($Prod['total']>0): ?>
                <li>Produits :
                    <ul>
                        <?
                        ...
                        ?>
                            <li>
                                <?=getinfo($prod['prod_id'],'products','name')?>
                                (<?=$prod['qte']?>)
                            </li>
                        <?endforeach;?>
                    </ul>
                ...
                <li>Filter pour les prospect invoquer
                    <ul>
                        ...
                        endforeach;
                        ?></li>
                        <li>Secteur IMS : <?$GouvInfo=  explode("@_@",$DetailInfo['secteur']);
